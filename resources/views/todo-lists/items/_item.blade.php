<div class="group py-3 flex items-center gap-3 border-b border-gray-100 dark:border-gray-800 last:border-b-0">
    {{-- Checkbox - 44px touch target --}}
    <form action="{{ route('todo-lists.items.toggle', [$list, $item]) }}" method="POST">
        @csrf
        <button
            type="submit"
            class="w-11 h-11 rounded-full border-2 flex items-center justify-center transition-colors {{ $item->completed ? 'bg-blue-500 border-blue-500' : 'border-gray-300 dark:border-gray-600 hover:border-blue-500' }}"
        >
            @if($item->completed)
